		...
		double static get_double_by_key(const char *dot_notation, bson_t &b, bool &success)
		{
			...
			if (bson_iter_init(&iter, &b) &&
				...
				if (!BSON_ITER_HOLDS_DOUBLE(&val)) {
					UE_LOG(LogROS, Warning, TEXT("Key found, but not double typed"));
				}
				else {
					//UE_LOG(LogROS, Verbose, TEXT("Key %s found. success is = %d"), *FString(UTF8_TO_TCHAR(dot_notation)), success);
					//UE_LOG(LogROS, Verbose, TEXT("Key %s found."), *FString(UTF8_TO_TCHAR(dot_notation)));
					success = true;
					//UE_LOG(LogROS, Verbose, TEXT("								 success is now = %d"), success);
					return bson_iter_double(&val);
				}
			}
			UE_LOG(LogROS, Display, TEXT("Couldn't find descendant"));
			success = false;
			return -1.0;
		}
